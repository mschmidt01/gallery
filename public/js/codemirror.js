...
window.CodeMirror = function () {
  ...
  var gecko = /gecko\/\d/i.test(navigator.userAgent); // IE11 currently doesn't count as 'ie', since it has almost none of
  ...
  function buildLineElement(cm, line, lineNo, dims, reuse) {
    ...
    if (!cm.options.lineNumbers && !markers && !bgClass && !line.wrapClass && !line.widgets) return lineElement; // Lines with gutter elements, widgets or a background class need
  ...
  function measureLineInner(cm, line) {
    ...
    var pre = buildLineContent(cm, line, measure, true).pre; // IE does not cache element positions of inline elements between
    ...
    for (var i = 0, cur; i < measure.length; ++i) {
      if (cur = measure[i]) {
        var node = cur,
            rect = null; // A widget might wrap, needs special care
  ...
  function endOperation(cm) {
    ...
    if (op.updateScrollPos) {
      ...
      alignHorizontally(cm);
      if (op.scrollToPos) scrollPosIntoView(cm, _clipPos(cm.doc, op.scrollToPos.from), _clipPos(cm.doc, op.scrollToPos.to), op.scrollToPos.margin);
  ...
  function registerEventHandlers(cm) {
    ...
    on(window, "resize", onResize); // Above handler holds on to the editor and its data structures.
  ...
  function setSelection(doc, anchor, head, bias, checkAtomic) {
    ...
    var sel = doc.sel;
    sel.goalColumn = null;
    if (bias == null) bias = posLess(head, sel.head) ? -1 : 1; // Skip over atomic spans.
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    if (indentString != curSpaceString) _replaceRange(cm.doc, indentString, Pos(n, 0), Pos(n, curSpaceString.length), "+input");else if (doc.sel.head.line == n && doc.sel.head.ch < curSpaceString.length) setSelection(doc, Pos(n, curSpaceString.length), Pos(n, curSpaceString.length), 1);
  ...
  CodeMirror.prototype = {
    ...
    scrollIntoView: operation(null, function (range, margin) {
      ...
      if (range.from.line != null) {
        ...
        coords = {
          from: _cursorCoords(this, range.from),
          to: _cursorCoords(this, range.to)
        };
      }
  ...
  CodeMirror.isWordChar = isWordChar; // MODE STATE HANDLING
  ...
  function buildTokenMeasure(builder, text, style, startStyle, endStyle) {
    ...
    for (var i = 0; i < text.length; ++i) {
      ...
      if (old) span.leftSide = old.leftSide || old; // In IE single-space nodes wrap differently than spaces
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    setSelection: docOperation(function (anchor, head, bias) {
      setSelection(this, _clipPos(this, anchor), _clipPos(this, head || anchor), bias);
    }),
    extendSelection: docOperation(function (from, to, bias) {
      extendSelection(this, _clipPos(this, from), to && _clipPos(this, to), bias);
    }),
  ...
  var isExtendingChar = /[\u0300-\u036F\u0483-\u0487\u0488-\u0489\u0591-\u05BD\u05BF\u05C1-\u05C2\u05C4-\u05C5\u05C7\u0610-\u061A\u064B-\u065F\u0670\u06D6-\u06DC\u06DF-\u06E4\u06E7-\u06E8\u06EA-\u06ED\uA66F\u1DC0–\u1DFF\u20D0–\u20FF\uA670-\uA672\uA674-\uA67D\uA69F\udc00-\udfff\uFE20–\uFE2F]/; // DOM UTILITIES
  ...
  };else if (webkit && /Chrome\/(?:29|[3-9]\d|\d\d\d)\./.test(navigator.userAgent)) spanAffectsWrapping = function spanAffectsWrapping(str, i) {
...
/***/ 4:
/*!******************************************!*\
  !*** multi ./resources/js/codemirror.js ***!
  \******************************************/
/*! no static exports found */
